...
class PatchEncoder(nn.Module):
    """
    A patch encoder that does a linear projection of patches in an RGB image.
    """
...
class PatchEncoder3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(
                C, 64, kernel_size=(T, 7, 7), stride=(T, 2, 2), padding=(0, 3, 3), bias=False
            ),
            nn.BatchNorm3d(
                64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True
            ),
        ...
        self.bn = nn.BatchNorm3d(embed_size)

    def forward(self, x):
        B, T, C, H, W = x.shape
        x = self.conv(x.transpose(1, 2))
        x = self.proj(x)
        x = self.bn(x)  # B, C, T, H, W
