...
from ladiwm.dataloader import BCDataset, get_dataloader, BCDataset3Real
...
def main(cfg: DictConfig):
    ...
    train_dataset = BCDataset3Real(dataset_dir=cfg.train_dataset, **cfg.dataset_cfg, aug_prob=cfg.aug_prob)
    # train_dataset, val_dataset = train_dataset.split(ratio=cfg.train_val_split)
    train_loader = get_dataloader(train_dataset,
                                      mode="train",
                                      num_workers=cfg.num_workers,
                                      batch_size=cfg.batch_size)

    # train_vis_dataset = BCDataset3Real(dataset_dir=cfg.train_dataset, vis=True, **cfg.dataset_cfg, aug_prob=cfg.aug_prob)
    # train_vis_dataloader = get_dataloader(train_vis_dataset,
    #                                           mode="train",
    #                                           num_workers=1,
    #                                           batch_size=1)

    val_dataset = BCDataset3Real(dataset_dir=cfg.val_dataset, num_demos=cfg.val_num_demos, **cfg.dataset_cfg, aug_prob=0.)
    val_loader = get_dataloader(val_dataset, mode="val", num_workers=cfg.num_workers, batch_size=cfg.batch_size)

    # val_vis_dataset = BCDataset3Real(dataset_dir=cfg.val_dataset, num_demos=cfg.val_num_demos, vis=True, **cfg.dataset_cfg, aug_prob=0.)
...
def evaluate(model, dataloader, mix_precision=False, tag="val"):
    ...
    for obs, track_obs, track, task_emb, action, extra_states in tqdm(dataloader):
        ...
        _, ret_dict = model.evaluate_loss(obs, track_obs, track, task_emb, extra_states, action)
