...
from ladiwm.utils.flow_utils import sample_from_mask, sample_double_grid
from ladiwm.utils.cotracker_utils import Visualizer
...
def get_task_bert_embs(libero_root_dir):
    ...
    tasks_exist = list(task_name_to_emb.keys())
    additional_keys = []
    for k in task_names:
        if k not in tasks_exist:
    ...
    if len(additional_keys) > 0:
        ...
        task_embs_add = get_task_embs(cfg, additional_keys).cpu().numpy()

        task_name_to_emb_add = {additional_keys[i]: task_embs_add[i] for i in range(len(additional_keys))}
        task_name_to_emb.update(task_name_to_emb_add)
        np.save("libero/task_embedding_caches/task_emb_bert.npy", task_name_to_emb)
...
def track_through_video(video, track_model, num_points=1000):
    ...
    # video = torch.nn.functional.interpolate(video, (H//4, W//4))
    # H, W = H//4, W//4
...
def collect_states_from_demo(h5_file, image_save_dir, demos_group, demo_k, view_names, track_model, task_emb, num_points, visualizer, save_vis=False):
    ...
    for view in view_names:
        ...
        pred_tracks = torch.rand(1, T, 100, 2).cuda()
        pred_vis = (torch.rand(1, T, 100)>0.5).cuda()
...
@click.option("--suite", type=str, default="libero_object")
...
def main(root, save, suite, skip_exist):
    ...
    # setup cotracker
    cotracker = torch.hub.load(os.path.join(os.path.expanduser("~"), ".cache/torch/hub/facebookresearch-co-tracker-9ed0531/"), "cotracker2", source="local")
