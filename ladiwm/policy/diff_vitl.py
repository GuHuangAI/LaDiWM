...
class BCViLTPolicyDiff(nn.Module):
    """
    Input: (o_{t-H}, ... , o_t)
    ...
    def __init__(self, obs_cfg, img_encoder_cfg, language_encoder_cfg, extra_state_encoder_cfg, track_cfg,
                 ...
                 policy_head_cfg, load_path=None, sampling_step=1):
        super().__init__()
    ...
    def spatial_encode2(self, time, obs, track_obs, task_emb, extra_states, return_recon=False):
        """
        ...
        for view_idx in range(self.num_views):
            img_encoded.append(self.image_encoders[view_idx](obs[:, view_idx, ...])
                # rearrange(
        ...
        img_encoded = torch.cat(img_encoded, -2)  # (b, 2*num_patches, c)
        img_encoded += self.img_patch_pos_embed  # (b, 2*num_patches, c)
        B, N = img_encoded.shape[:2]
        ...
        out = self.spatial_transformer(img_track_text_encoded)
        # out = out[:, 0]  # extract spatial token as summary at o_t
        out = self.spatial_downsample(out) # (b, 2*num_img_patch + 1, c')
        ...
        text_encoded_ = text_encoded_.view(B, 1, -1)  # (b, 1, c')
        ...
        # for _ in out_seq:
        #     print(_.shape)
        time_emb = self.time_embed(time.expand(B)).unsqueeze(1)
        out_seq.append(time_emb)
        output = torch.cat(out_seq, -2)  # (b, num, c')
    ...
    def temporal_encode(self, x):
        ...
        x = x + pos_emb.unsqueeze(0)  # (b, t, 2+num_extra, c)
        sh = x.shape
        self.temporal_transformer.compute_mask2(x.shape)
    ...
    def sample_act(self, obs, track_obs, track, task_emb, extra_states):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            x_cur = x_next
            pred = self.forward_sample(x_cur, t_cur, obs, track_obs, track, task_emb, extra_states)
            C, noise = pred[:2]
    ...
    def forward(self, action, obs, track_obs, track, task_emb, extra_states, mode='train'):
        """
        ...
        # obs = obs[:, :, -10:]
        x = self.spatial_encode2(time, obs, track_obs, task_emb, extra_states, return_recon=False)  # x: (b, t, 2+num_extra, c), recon_track: (b, v, t, tl, n, 2)
    ...
    def forward_sample(self, x_cur, t_cur, obs, track_obs, track, task_emb, extra_states):
        # b = obs.shape[0]
        # obs_view1 = obs[:, 0]
        # obs_view2 = obs[:, 1]
        # with torch.no_grad():
        ...
        # obs = torch.stack([obs_view1, obs_view2], dim=1)  # b v t c h w
        # obs = obs[:, :, -10:]
        x = self.spatial_encode2(t_cur, obs, track_obs, task_emb, extra_states,
    ...
    def forward_loss(self, obs, track_obs, track, task_emb, extra_states, action):
        ...
        loss = self.forward(action, obs, track_obs, track, task_emb, extra_states)
    ...
    def act(self, obs, task_emb, extra_states):
        ...
        # obs = obs[:, :, -10:]
        with torch.no_grad():
            action = self.sample_act(obs, track_obs, None, task_emb=task_emb, extra_states=extra_states)
        ...
        return action.float().cpu().numpy()[:, 0], pred_view
