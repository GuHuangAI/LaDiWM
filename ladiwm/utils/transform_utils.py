...
def mat2quat_torch(rmat):
    ...
    for i in range(rmat.shape[0]):
        ...
        inds = torch.tensor([3, 0, 1, 2], device=rmat.device)
        q1 = V[inds, torch.argmax(w)]
        if q1[0] < 0.0:
            q1 = torch.negative(q1)
        inds = torch.tensor([1, 2, 3, 0], device=rmat.device)
        q1 = q1[inds]
        out.append(q1)
...
def quat2mat_torch(quaternion):
    ...
    for i in range(quaternion.shape[0]):
        ...
        q = quaternion[i].clone().to(torch.float32)[inds]
        n = torch.dot(q, q)
        if n < EPS:
            out_tmp = torch.eye(3, device=quaternion.device)
        q *= torch.sqrt(2.0 / n)
        q2 = torch.outer(q, q)
        out_tmp = torch.tensor(
            [
                [1.0 - q2[2, 2] - q2[3, 3], q2[1, 2] - q2[3, 0], q2[1, 3] + q2[2, 0]],
                [q2[1, 2] + q2[3, 0], 1.0 - q2[1, 1] - q2[3, 3], q2[2, 3] - q2[1, 0]],
                [q2[1, 3] - q2[2, 0], q2[2, 3] + q2[1, 0], 1.0 - q2[1, 1] - q2[2, 2]],
...
def quat2axisangle_torch(quat):
    ...
    for i in range(quat.shape[0]):
        q_tmp = quat[i]
        den = torch.sqrt(1.0 - q_tmp[3] * q_tmp[3])
        if torch.isclose(den, torch.tensor(0.0)):
            # This is (close to) a zero degree rotation, immediately return
            aa = torch.zeros(3).to(q_tmp.device)
        else:
            aa = (q_tmp[:3] * 2.0 * torch.acos(q_tmp[3])) / den
    # if math.isclose(den, 0.0):
    #     # This is (close to) a zero degree rotation, immediately return
    #     return np.zeros(3)
        out.append(aa)
...
def axisangle2quat_torch(vec):
    ...
    for i in range(vec.shape[0]):
        v_tmp = vec[i]
        angle = torch.norm(v_tmp)
        # handle zero-rotation case
        if math.isclose(angle, 0.0):
            q = torch.tensor([0.0, 0.0, 0.0, 1.0], device=v_tmp.device)
        else:
            # make sure that axis is a unit vector
            axis = v_tmp / angle

            q = torch.zeros(4).to(v_tmp.device)
